...
async def create_book(book: BookModel, user_id: uuid.UUID):
    ...
    async with  async_session_maker() as session:
        new_book = Book(book_id=str(uuid.uuid4()), name=book.name, author=book.author, user_id=user_id)
...
async def delete_book(book_id: uuid.UUID, user_id:uuid.UUID):
    ...
    async with  async_session_maker() as session:
        stmt = select(Book).where(Book.book_id == str(book_id)).where(Book.user_id == str(user_id))
        result = await session.execute(stmt)
